...
- **会话时间线** —— 长会话右缘的 DeepSeek 官网风格轮次导航轨：每一轮一个刻度，悬停或聚焦时预览该轮的问题。 两套前端、同一套后端，按当前内核支持的能力自动选择：   - **dsh ≥ 0.1.2（增强）** —— 内核已经自带这条导航轨，因此插件不渲染任何自己的界面，只修正它仍欠缺的行为（按导航轨自己发布的度量做能力探测，绝不比较版本号）： 对**已加载**轮次的点击由官方的瞬时跳转改为**平滑滑动**（缓入缓出、按距离定时长、读者一滚动立刻接管、`prefers-reduced-motion` 下直接定位）。 在 **0.1.2-alpha.1/2** 上导航轨只列出已加载的轮次，因此读者把手伸向导航轨时还会自动往回翻页加载历史，让“需要点加载更多才出现”的轮次也长出刻度、可以直接跳转 —— 补到导航轨自身不压缩的刻度上限为止（再往后官方样式表会把刻度挤成无法瞄准的实心条）。 在 **0.1.2-alpha.3+** 上官方重构了导航轨（固定间距刻度在框内滚动，数据来自全会话轮次大纲：每一轮都在轨上，未加载的轮次由内核自行翻页），插件相应收窄为只保留平滑滑动 —— 点击索引按内核渲染的同一份合并阶梯解析，已加载窗口之外的刻度交给内核自己的跳转机器，历史预热不再运行。官方悬浮预览卡是实心底，在壁纸门控下改为与输入卡同配方的毛玻璃（受「毛玻璃模糊」滑块驱动）；刻度列加上 DeepSeek 网页版式的上下渐隐（仅绘制层，不影响命中测试）。导航轨的高亮状态仍归内核自己维护——滑动走的是真实滚动容器，官方滚动处理会持续更新活动轮次与持久化滚动位置。   - **dsh ≤ 0.1.1（移植版）** —— 没有官方导航轨，插件渲染自己移植的同一套界面（完全相同的尺寸，同样带上玻璃 + 上下渐隐）， 数据来自同一套后端。 两种情况下，滑动前都会先解除宿主的底部跟随状态（流式增长、轮次结束重渲染都不会把视图拽回底部）；被 rewind 撤回的消息自动从轨道剔除； 凡插件自己翻页加载历史的地方（移植轨与 alpha.1/2 轨），每一页都会补偿前置内容的高度，读者视野纹丝不动。用设置行里的时间线开关可关闭。
...
| 会话时间线 | 会话时间线的开关（默认开启）。dsh ≥ 0.1.2 上该项显示为**会话时间线增强**：官方导航轨保留，插件只优化其行为，关掉即恢复官方原样（说明文案随内核代际分两版——alpha.1/2 完整增强，alpha.3+ 仅平滑跳转）；旧内核上该项为**会话时间线**，开关控制插件自己移植的导航轨。 |
...
- **会话时间线**注册进 `conversation.input.dock` 槽位（绑定每会话生命周期）。模式判定是对槽位 props 的能力探测，而不是比较版本号： 官方导航轨正是由 `ui-chat` 以会话 hook 形式发布的同一份索引渲染的（`useChat(s => s.navigation.items())`，dsh ≥ 0.1.2）， 所以这个 hook 存在**就是**官方导航轨存在 —— 而且它能扛住预发布版、fork 以及挂载了其它会话目标的部署。 hook 存在时插件什么都不渲染，只在**捕获阶段**拦截官方导航轨的点击（React 18 在冒泡阶段由根容器派发 `onClick`， 所以导航轨上的捕获监听先执行，`stopImmediatePropagation()` 让官方处理器根本收不到事件）——点击索引按内核实际渲染的阶梯解析：压缩式 alpha.1/2 轨上是已加载条目本身，alpha.3 帧式轨上则是与内核同构的「大纲 + 已加载」合并阶梯，已加载窗口之外（无锚键）的刻度交给内核自己的穿越加载跳转——且当内核跳转仍在翻页（标记以 `aria-busy` 脉冲）时插件整体让位：内核的已加载分支落点前会先取消自己的在途跳转，拦截会绕过这一取消；hook 不存在时插件渲染自己移植的导航轨，portal 到 `body`。 两者跑同一个跳转引擎。数据最快的来源是宿主侧会话投影（`bgTimeline`，注册于 `src/projection.ts`）， 它枚举整个会话的每一条用户消息（含会话视图尚未分页载人的轮次），已加载的 chat 节点窗口作为兜底； 该窗口以鸭子类型同时兼容两代内核，因为 0.1.2 把 Chat 快照从 `session.getSnapshot().chat` 迁到了 `useChat()` —— 这是本插件唯一一处破坏性变更。 历史预热只在读者把手伸向导航轨时运行——而且只在其仍有意义的地方运行：压缩式 alpha.1/2 轨与移植轨，并停在导航轨不压缩的刻度上限（`floor((高度 - 12) / 10) + 1`）； alpha.3 帧式轨已列出每一轮并自行按需翻页，预热只会与内核的翻页器相互踩踏，故不运行。 每一页都补偿前置内容的高度，因为宿主的 prepend 锚点在插件侧无法触及。移植版导航轨与增强后的官方轨采用完全一致的**玻璃 + 上下渐隐处理** —— 悬浮预览在壁纸门控下加入毛玻璃配方（与输入卡片同一显式填充配方），刻度列带 DeepSeek 网页版式上下渐隐，两个前端对读者的视觉完全一致。
...
│       │   ├── official-enhance.tsx # dsh>=0.1.2：平滑跳转拦截（+ alpha.1/2 轨历史预热）
