...
- **会话时间线** —— 长会话右缘的 DeepSeek 官网风格轮次导航轨：每一轮一个刻度，悬停或聚焦时预览该轮的问题。 本插件**不渲染任何自己的界面**——从 dsh 0.1.2-rc.1 起内核自带官方导航轨，插件只增强其行为（按导航轨自己发布的度量做能力探测，绝不比较版本号）： 对**已加载**轮次的点击由官方的瞬时跳转改为**平滑滑动**（缓入缓出、按距离定时长、读者一滚动立刻接管、`prefers-reduced-motion` 下直接定位）； 官方轨的全量阶梯（`turnOutline` 大纲 + 已加载窗口）中，**未加载**轮次的刻度交给内核自己的穿越加载跳转，且当内核跳转仍在翻页（`aria-busy` 脉冲）时插件整体让位； 官方悬浮预览卡是实心底，在壁纸门控下改为与输入卡同配方的毛玻璃（受「毛玻璃模糊」滑块驱动）； 刻度列加上 DeepSeek 网页版式的上下渐隐（仅绘制层，不影响命中测试）。 导航轨的高亮状态仍归内核自己维护——滑动走的是真实滚动容器，官方滚动处理会持续更新活动轮次与持久化滚动位置； 滑动前都会先解除宿主的底部跟随状态（流式增长、轮次结束重渲染都不会把视图拽回底部）。用设置行里的时间线开关可关闭。
...
| 本插件版本 | 支持的 dsh 版本 |
| --- | --- |
| **当前版本（≥ 0.5.3）** | **dsh 0.1.2-rc.1 与 0.1.3-alpha.x（已在 alpha.2 验证），另支持 0.1.5-alpha.1 / alpha.2 / rc.1 / rc.2（已在 rc.2 验证）与 0.1.6-alpha.1 / alpha.2** |
| 0.5.2 | dsh 0.1.1-rc.2 与 0.1.2-alpha.1 ~ alpha.5 |

⚠️ **使用旧版 dsh（0.1.1-rc.2、0.1.2-alpha.x 或更早）请安装本插件的旧版本**
（`dsh plugin --profile web add deepseek-harness-background@0.5.2`），本插件已停止对旧内核的适配。
...
│       ├── background-css.ts # 注入的样式表（层、玻璃、明暗遮罩、变量、read_image 卡）
│       ├── glass-registry.ts # 第三方毛玻璃注册表（window 桥 + 门控规则合成）
│       ├── timeline/
│       │   ├── index.tsx            # 停靠入口：官方轨能力探测与再导出
│       │   ├── official-enhance.tsx # dsh>=0.1.2-rc.1：已加载刻度平滑跳转拦截
│       │   ├── jump.ts              # 共享跳转引擎（翻页 + 滑动）
│       │   ├── rail-pointer.ts      # 帧式刻度几何与全量阶梯合并
│       │   └── types.ts             # 共享类型契约
│       ├── SettingsRow.tsx   # 通用设置中的设置行（预览卡 + 阻尼滑块）
│       ├── SettingsRow.module.css # 设置行样式（官方 token）
│       ├── settings-client.ts# fetch 传输层（读/写/上传）
│       └── locales.ts        # 中/英文案
└── tests/                  # 契约测试（schema、routes、apply/painter、设置行、时间线）
```
