 ...
 *
 * Two official rail generations publish different geometries, told apart by
 * capability (an inline custom property), never by version compare:
 * - alpha.1/2: marks position themselves by ratio and the rail COMPRESSES
 *   past its capacity — a click maps by pointer ratio across the column.
 * - alpha.3: fixed 10px pitch, marks scroll INSIDE the frame, and the
 *   scroller offset is published inline as `--turn-scroll-top` — a click
 *   maps by pitch with that offset added.
 */
...
/**
 * Resolve the entry under a pointer Y on the COMPRESSING rail — the official
 * alpha.1/2 geometry verbatim (`itemAtPointer` in that generation's
 * TurnNavigator.tsx), so this plugin lands on exactly the mark the official
 * rail would have picked. The legacy port (whose own rail never scrolls
 * internally) uses the same mapping, where ratio and fixed pitch coincide.
 ...
 * resolves through the rail geometry — the frame-style alpha.3 mapping when
 * the rail publishes `--turn-scroll-top`, the compressing alpha.1/2 mapping
 * otherwise. Returns -1 when neither applies.
 *
...
export function indexForEvent(rail: HTMLElement, event: MouseEvent): number {
  ...
  if (count === 0) return -1
  return isFrameRail(rail)
    ? frameIndexAtPointer(count, rail, event.clientY)
    : indexAtPointer(count, rail, event.clientY)
}
